<div align="center">
  
  <h1>Hola, soy VISION</h1>

  <p>
    <strong>Andrés Felipe Quiroz Pérez </strong> <br>
    Desarrollador Full-Stack (Java/React) | Integración de IA & Backend Architecture
  </p>
  <!-- Banner horizontal de VISION -->
  <img src="image-github-vision.png" alt="VISION - Estudio de innovación y construcción digital" width="600" style="margin: 20px 0;" />

  <br />

  Soy un desarrollador orientado al pensamiento de negocio, especializado en la creación de aplicaciones modulares y escalables. Combino el desarrollo backend tradicional con implementaciones avanzadas de Inteligencia       Artificial (Agentes Autónomos, RAG, Búsqueda Semántica) para resolver problemas reales.

  Actualmente soy estudiante destacado (1er puesto de cohorte por 4 semestres) en la Universidad del Valle y ponente en investigación tecnológica (RREDSI).
</div>

### 🚀 Stack Principal y Tecnologías

*   **Backend:** Java 17, Spring Boot 3.5, Spring Security (JWT), Hibernate, JPA.
*   **Frontend:** React 19, TypeScript, TailwindCSS, Arquitectura Feature-based.
*   **IA & Datos:** LangChain4j, PostgreSQL + PgVector (Embeddings), OpenAI, Gemini.
*   **Arquitectura:** AOP, Spring Retry, Procesamiento Asíncrono, Tool Calling.
*   **Testing & DevOps:** JUnit 5, Mockito, TestContainers, Docker, Git.

### 🏆 Proyecto Destacado: OrchestrAI
Plataforma full-stack de orquestación asistida por IA.
*   **Pipeline RAG completo:** Ingesta de documentos, chunking, embeddings y búsqueda semántica con filtro híbrido en PostgreSQL + PgVector.
*   **Agente Autónomo:** Tool Calling dinámico con reintentos lógicos (Spring Retry + AOP) e integraciones externas (API de Trello).
*   **Seguridad y Control:** Autenticación stateless con JWT, RoleGuard en frontend y manejo centralizado de excepciones personalizadas.

### 📈 Logros Relevantes
- 🥇 **Mérito Académico:** 1er Puesto y Mejor Promedio de Cohorte (4 semestres consecutivos, Univ. del Valle).
- 🎤 **Investigación:** Ponente en el XV Encuentro RREDSI por el desarrollo de la plataforma OrchestrAI (Calificación: 93.5/100 - Ranking #26).
- 💼 **Experiencia:** +2 años en entornos de teletrabajo colaborando como analista de diseño y ejecución autónoma de entregables.

---
📫 **¿Conectamos?**  andresfqperez@gmail.com
